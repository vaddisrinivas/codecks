...
data class DeckLayout(
    ...
    fun replacingAction(actionId: String, action: DeckAction): DeckLayout =
        copy(slots = slots.map { slot -> if (slot.action.id == actionId) slot.copy(action = action) else slot })
