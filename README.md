...
`v0.1.18` is the current public beta. Core deck, trackpad, keyboard, clipboard, rules, editing, settings, optional AI-assisted drafting, and default-off Smart suggestions are implemented in the single signed APK. Broader physical-device coverage, TalkBack validation, and longer crash-free field testing remain GA gates. See the [production launch plan](docs/release/PRODUCTION_LAUNCH_PLAN.md).
