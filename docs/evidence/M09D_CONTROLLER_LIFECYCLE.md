...
1. Prior source `f3b2a29a2c98ddace4ac9d4ad87122197c49d745` preserves the reviewed raw-cleanup source. This append-only truth-narrowing source phase changes exactly six evidence paths and is direct-parented by `f3b2a29`.
2. C2 artifacts: from clean truth-narrowing source, run the exact filtered Gradle command and commit only deterministic sanitized JUnit XML, canonical sanitized log, and artifact manifest; C2 is direct-parented by truth-narrowing source.
