...
| --- | --- | --- |
| removed `ConnectionScreen` | M02 found zero production, manifest, route, worker, serializer, JNI, reflection, preview, or DI reachability. Two source-reading tests now target the active Settings owner. | `DEAD_CONFIRMED`; deleted. Replacement and rollback mapping are in `tasks/test-evidence/autonomous-maturity-m02-dead-surfaces.json`. |
| removed `DevicesScreen` and `DevicesViewModel` | M02 found definition-only source reachability. The generated Hilt binding had no `viewModel()` retrieval, factory, route, manifest, serializer, worker, JNI, reflection, or migration consumer. | `DEAD_CONFIRMED`; deleted. Active Mac selection/setup remains in Settings through `ConnectionViewModel` and `ConnectionRepository`. |
| private preview functions in `ui/quality/AdaptiveSurfacePreviews.kt` | File contains six intentional `@Preview` entry points for phone, foldable, and tablet tooling. It has no runtime entry point. | `RETAINED_TOOLING`; runtime-unreachable is not sufficient proof of dead code because Compose tooling discovers the annotations. |
